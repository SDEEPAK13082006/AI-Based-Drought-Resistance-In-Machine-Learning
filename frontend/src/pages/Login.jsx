  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        // Real backend responded with an error
        setError(err.response.data.detail);
      } else if (!err.response) {
        // Network error — backend is unreachable, fall back to mock auth
        try {
          const mockData = mockLogin(cleanUsername, cleanPassword);
          login(mockData.access_token, mockData.username);
          navigate('/dashboard');
        } catch (mockErr) {
